...
export function createCoworkUI(api) {
  ...
  function renderWork(){
    ...
    return `<div class="cw-back"><a href="#home" class="link">← 返回工作台</a><span>${esc(c.city)} · ${esc(c.timezone)} · v${w.revision}</span></div>${heading(esc(w.goal),`${esc(c.name)} · ${esc(w.plan.label)} · 本地持续工作记录`,`${btn('和 Cowork 聊聊','chat-open','message','primary',`data-customer="${c.id}"`)}${status(w)}${action('客户画像','cw-customer',w,'users')}`)}
