...
export function createChatUI(api) {
  ...
  function renderConversation(chat,home=false){
    ...
    return `<section id="cowork-conversation" data-conversation-id="${esc(chat?.id||'new')}" class="chat-session ${home?'xp-home-conversation':'xp-cowork-panel'} ${items.length?'has-messages':''}" aria-label="Cowork 对话"><div class="xp-cowork-heading"><span class="xp-symbol">${i('spark')}</span><div><strong>${home?'和 Cowork 一起工作':'Cowork'}</strong><small>${home?(chat?.profileCapture?'记录客户沟通，持续完善画像。':'问问题、写话术、记录反馈，也可以直接交代工作。'):'围绕当前成果，一起修改和完善'}</small></div><div class="row">${home?btn('新对话','chat-new','plus','sm'):''}${btn('参考资料','chat-details','book','sm')}</div></div><div class="chat-session-top"><label class="chat-context-label">${i('book')}<select id="cowork-chat-customer" aria-label="本次对话关联客户" ${running?'disabled':''}>${options(chat?.customerId||'')}</select></label>${home&&items.length?btn('转为任务','chat-track','target','sm',running?'disabled':''):''}</div><div id="cowork-chat-connection">${renderConnection()}</div>${ui().chatDetails?renderSources(chat,c,running):''}<div class="chat-messages" role="log" aria-label="对话记录" aria-live="polite">${items.length?items.map(m=>renderMessage(m,chat)).join(''):home?`<div class="xp-home-prompts"><button data-act="chat-example" data-example="为什么客户试驾后不回复？">讨论一个问题</button><button data-act="chat-example" data-example="查找客户 RAV4">查找客户资料</button><button data-act="chat-example" data-example="帮 Sarah 写一句简短的英文跟进话术">写一句话术</button>${btn('记录客户沟通','chat-profile','users','sm')}</div>`:`<div class="chat-welcome"><h2>这次想一起完成什么？</h2><p>补充工作要求或新的反馈，我会据此准备和调整成果。</p></div>`}<div id="chat-end"></div></div>${!home?workbench.renderTarget():''}<form id="cowork-chat-form" class="chat-composer chat-bottom-composer"><label class="sr-only" for="cowork-chat-input">给 Cowork 发消息</label><textarea id="cowork-chat-input" maxlength="6000" rows="3" placeholder="${home?'例如：帮我写一句跟进话术，或根据这个线索制定转化方案…':(workbench.selected()?(workbench.selected().format==='poster'?'例如：精简当前海报的标题，保留活动信息…':'例如：把当前文档的跟进步骤写得更具体…'):'描述你要完成的工作和希望得到的成果…')}">${esc(drafts.get(chat?.id||'new')||'')}</textarea><div class="chat-composer-bottom"><span>Enter 发送 · Shift + Enter 换行</span>${running?btn('停止','chat-stop','pause','soft'):'<button class="btn primary" type="submit">'+i('arrow')+'发送</button>'}</div></form></section>`;
  }
