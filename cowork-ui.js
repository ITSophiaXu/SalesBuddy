...
import {visibleWork} from './workspace-model.js';
...
export function createCoworkUI(api) {
  ...
  function renderWork(){
    ...
    return `<div class="cw-back"><a href="#desk" class="link">← 返回工作台</a><span>${esc(c.city)} · ${esc(c.timezone)} · v${w.revision}</span></div>${heading(esc(w.goal),`${esc(c.name)} · ${esc(w.plan.label)} · 本地持续工作记录`,`${btn('和 Cowork 聊聊','chat-open','message','primary',`data-customer="${c.id}"`)}${status(w)}${action('客户画像','cw-customer',w,'users')}`)}
  ...
  function renderInbox(){
    const list=scoped().filter(w=>visibleWork(w)&&w.activated&&openHuman(w.human)&&!['paused','declined'].includes(w.status));
    if(!list.length||ui().todoFilter==='done')return '';
    const filtered=list.filter(w=>!ui().todoMine||w.human.owner===state().settings.operator);
    return filtered.length?'<details class="ux-legacy-todos"><summary>客户工作中的已有安排 · '+filtered.length+'</summary><div class="cw-inbox-grid">'+filtered.map(w=>humanCard(w,true)).join('')+'</div></details>':'';
  }
  ...
  async function handleAction(act,data){
    ...
    try{
      ...
      if(act==='cw-open'){if(w){w.activated=true;api.save();}ui().workId=data.id;api.closeModal();api.navigate('work');return true;}
