...
test('截图中的六个入口各有唯一页面归属，主动工作和生态配置仍可访问',()=>{
  ...
  assert.ok(PRIMARY_NAV.find(n=>n.id==='desk').pages.includes('proactive'));
  assert.ok(PRIMARY_NAV.find(n=>n.id==='connections').pages.includes('ecosystem'));
});
